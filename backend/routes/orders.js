...
router.get('/', authenticateToken, roleAuthentication(ROLES.CLIENT, ROLES.ADMIN), async (req, res) => {
    try {
        ...
        if (req.user.role == ROLES.CLIENT){
            ...
            if (!filtered_orders) {
                Logger.info("No orders found for logged in user");
                return res.status(400).json({ message: "No orders found for the logged in user" });
            }
...
router.post('/', authenticateToken, roleAuthentication(ROLES.CLIENT), async (req, res) => {
    ...
    const start = async () => {
        await asyncForEach(products, async (product_id) => {
          ...
          if (!temp) {
              Logger.info(`Product:${product_id} does not exist`);
              //console.log(`Product:${product_id} does not exist`);
          }
        });
        ...
        try {
            ...
            if (!data) {
                ...
                return res.status(400).json({ message: "Failed saving the order" });
            }
